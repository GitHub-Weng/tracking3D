...
//这里如果注释掉的话就不会有TabItem被选中的效果,so we use the super method to get the right VCs
...
//            id tempVC = viewControllers[OneDimension].childViewControllers.lastObject;
//
//            if([tempVC isKindOfClass:[LLAPViewController class]])
//            {
//                LLAPAudioController* llapvc = [(LLAPViewController *)tempVC getLLapVC];
//
//            }
        }
            break;
        case Visualized:
        {
//            id tempVC = self.viewControllers[OneDimension].childViewControllers.lastObject;
//            if([tempVC isKindOfClass:[AudioController class]]){
//                AudioController *audioVC = (AudioController *)tempVC;
//                [audioVC setupIOUnit];
//                //[audioVC createButtonPressedSound];
//                [(EAGLView *)audioVC.view setupViewRefresh];
//            }
        }
            break;
        case TwoDimension:
