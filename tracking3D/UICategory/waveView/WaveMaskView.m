...
@interface WaveMaskView() {
    CGFloat a; // 决定峰值；这个值变化，视觉上会呈现出不同的波动速度
    BOOL toAdd; // 形成上下波动的视觉效果
    
    CGFloat offset;
    CGFloat waveWidth; // 水波的宽度（即 WaveView 实例对象的宽）
...
- (instancetype)initWithFrame:(CGRect)frame bottomImage:(UIImage*)bottomImage upImage:(UIImage*)upImage {
    if (self = [super initWithFrame:frame]) {
        [self commonInit];
        
        self.bottomImgView.image = bottomImage;
        self.upImgView.image = upImage;
    }
    return self;
}
