...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super init]) {
        ...
        _buttonA = [[AAPLButtonOverlay alloc] initWithText:@"Jump"];
        _buttonB = [[AAPLButtonOverlay alloc] initWithText:@"Attack"];
        ...
        _buttonA.position = CGPointMake(
            center.x + buttonDistance * cosf(ToRadian(ButtonAPositionAngleDegree)) - buttonCenterOffset.x + 100,
            center.y - 100 + buttonDistance * sinf(ToRadian(ButtonAPositionAngleDegree)) - buttonCenterOffset.y);
        [self addChild:_buttonA];

        _buttonB.position = CGPointMake(
            center.x + buttonDistance * cosf(ToRadian(ButtonBPositionAngleDegree)) - buttonCenterOffset.x + 90,
            center.y - 100 + buttonDistance * sinf(ToRadian(ButtonBPositionAngleDegree)) - buttonCenterOffset.y);
        [self addChild:_buttonB];
    }
    return self;
}
- (AAPLButtonOverlay*)getAAPLButtonOverlay:(NSString*)buttonName_JumpOrAttack
{
    if([buttonName_JumpOrAttack isEqualToString:@"Jump"])
    {
        if(_buttonA){
            return _buttonA;
        }
    }
    if([buttonName_JumpOrAttack isEqualToString:@"Attack"]){
        if(_buttonB){
            return _buttonB;
        }
    }
